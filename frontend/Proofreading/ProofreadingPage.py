    ...
    def load_data(self) -> None:
        ...
        def task() -> None:
            ...
            try:
                ...
                self._last_loaded_count = len(items)

                self.items_loaded.emit(items)

            except Exception as e:
                self._last_load_error = str(e) if e else ""
    ...
    def _on_items_loaded_ui(self, items: list[Item]) -> None:
        ...
        if items:
            self.emit(Base.Event.TOAST, {
                ...
                "message": Localizer.get().proofreading_page_load_success.replace("{COUNT}", str(self._last_loaded_count or len(items))),
            })
            self._apply_filter()
        else:
            if self._last_load_error:
                detail = self._last_load_error.replace("\n", " ").strip()
                if len(detail) > 200:
                    detail = detail[:200] + "..."
                self.emit(Base.Event.TOAST, {
                    "type": Base.ToastType.ERROR,
                    "message": f"{Localizer.get().proofreading_page_load_failed}: {detail}",
                })
            # 清空数据并显示占位符
            self.table_widget.set_items([], {})
