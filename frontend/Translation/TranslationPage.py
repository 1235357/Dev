...
class TranslationPage(QWidget, Base):
    ...
    def update_ui_tick(self) -> None:
        ...
        self.update_zh_chars(self.data)
    ...
    def update_button_status(self, event: Base.Event, data: dict) -> None:
        ...
        if event == Base.Event.PROJECT_CHECK_DONE and isinstance(data.get("extras"), dict) and data.get("extras"):
    ...
    def update_time(self, data: dict) -> None:
        status = Engine.get().get_status()
        if status in (Base.TaskStatus.STOPPING, Base.TaskStatus.TRANSLATING):
            ...
            if total_time == 0 and self.data.get("start_time", 0) == 0:
                return None
    ...
    def update_line(self, data: dict) -> None:
        if self.data.get("total_line", 0) == 0 and self.data.get("line", 0) == 0:
            return None
    ...
    def add_widget_body(self, parent: QLayout, config: Config, window: FluentWindow) -> None:
        ...
        self.add_zh_chars_card(self.flow_layout, config, window)
    ...
    def add_zh_chars_card(self, parent: QLayout, config: Config, window: FluentWindow) -> None:
        self.zh_chars = DashboardCard(
            parent = self,
            title = Localizer.get().translation_page_card_zh_chars,
            value = Localizer.get().none,
            unit = "",
        )
        self.zh_chars.setFixedSize(204, 204)
        parent.addWidget(self.zh_chars)

    def update_zh_chars(self, data: dict) -> None:
        if self.data.get("start_time", 0) == 0:
            self.zh_chars.set_unit("")
            self.zh_chars.set_value(Localizer.get().none)
            return None

        zh_chars = int(self.data.get("total_zh_chars", 0) or 0)
        if zh_chars < 1000:
            self.zh_chars.set_unit("字")
            self.zh_chars.set_value(f"{zh_chars}")
        elif zh_chars < 1000 * 1000:
            self.zh_chars.set_unit("K字")
            self.zh_chars.set_value(f"{(zh_chars / 1000):.2f}")
        else:
            self.zh_chars.set_unit("M字")
            self.zh_chars.set_value(f"{(zh_chars / 1000 / 1000):.2f}")
