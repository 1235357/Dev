...
class LocalizerZH():
    ...
    translation_page_indeterminate_stopping: str = "正在停止翻译任务 …"
    ...
    translation_page_card_zh_chars: str = "总译文字数"
    ...
    proofreading_page_load_success: str = "缓存载入成功，共 {COUNT} 条"
    ...
    expert_settings_page_result_checker_retry_count_threshold: str = "结果检查 - 达到重试次数阈值"
    expert_settings_page_result_checker_retry_count_threshold_desc: str = (
        "在结果检查报告中包含 <font color='darkgoldenrod'><b>达到重试次数阈值</b></font> 的条目列表，默认禁用"
        "<br>"
        "• 在翻译结果检查过程中，如果条目在达到重试次数阈值后仍然未通过检查，将使用最后一次的翻译结果"
        "<br>"
        "• 启用此功能可以方便您单独检查最终采用的结果是否正确"
    )

    expert_settings_page_request_max_retries_title: str = "请求重试次数"
    expert_settings_page_request_max_retries_desc: str = (
        "网络请求失败时的客户端内部重试次数，0 = 不重试"
        "<br>"
        "流式传输异常的重试由“流式重试次数”控制"
    )
    expert_settings_page_stream_stall_timeout_seconds_title: str = "流式无数据超时（秒）"
    expert_settings_page_stream_stall_timeout_seconds_desc: str = (
        "流式响应已开始（收到过 chunk）后，连续多久未收到任何 chunk 则判定为中断"
        "<br>"
        "达到阈值会触发异常并进入重试流程"
    )
    ...
    expert_settings_page_stream_retry_backoff_seconds_title: str = "流式重试等待基数（秒）"
    ...
    glossary_page_table_row_04: str = "来源"
